...
uint8_t write_file_sequence[] = {
    SEQ_ITEM_TAG, 3, CMD_BYTE_WRITE, 0x40, 0x00,    // request to write 64 bytes
    ...
    // SEQ_ITEM_TAG, 1, just write data, no command    //
    SEQ_ITEM_TAG, 1, CMD_RD_USB_DATA0,              // actually read the data
    SEQ_SCBK_TAG, 0,                                // handle the result
    };
...
static int8_t sendCommand(ch376s_context_t *context, uint8_t cmd, const uint8_t *data, uint8_t length)
{
    ...
    int8_t rc = 0;
    // if (cmd != just write data)
        rc = write(command_buffer, 3);
